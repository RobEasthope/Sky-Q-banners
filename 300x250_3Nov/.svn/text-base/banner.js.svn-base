...
////
var stageP, spriteSheet, count = 0, myVar;
...
function initDynamicContent() {
    ...
    devDynamicContent.Feed_300x250[0].reporting_label = "REPORTING_LABEL";
    ...
    devDynamicContent.Feed_300x250[0].youtubeId = "-Bpm35wZ0us";
    devDynamicContent.Feed_300x250[0].content = {
        "Spritesheet_min.jpg":{
            "Type":"file",
            "Url":"Spritesheet_min.jpg"},
...
//    devDynamicContent.Feed_300x250[0].legalsCopy = "Comparison against Netflix and LOVEFiLM Instant using top 100 from UK box office over 12 months.  See sky.com\/blockbusters. On Demand: Available to Sky TV customers with compatible Sky+ box. Downloading content requires compatible black Sky+ box, broadband connection (minimum recommended speed 2 Mbps).  Content depends on your Sky TV\/HD subscription. \u00A9 2014 Disney Enterprises, Inc. All Rights Reserved. \u00A9 2014 Disney Enterprises, Inc. \u00A9 2015 Marvel .";
    ...
    document.getElementById("middle_container").innerHTML+="<img src="+dynamicContent.Feed_300x250[0].content['frame_1_copy.png']['Url']+" id='frame_1_copy' class='images fade'></img>";
    
    document.getElementById("middle_container").innerHTML+="<img src="+dynamicContent.Feed_300x250[0].content['frame_2_copy.png']['Url']+" id='frame_2_copy' class='images fade'></img>";
    
    document.getElementById("middle_container").innerHTML+="<img src="+dynamicContent.Feed_300x250[0].content['frame_3_copy.png']['Url']+" id='frame_3_copy' class='images fade'></img>";
    
    document.getElementById("middle_container").innerHTML+="<img src="+dynamicContent.Feed_300x250[0].content['ld_copy.png']['Url']+" id='ld_copy' class='images fade'></img>";
    
    document.getElementById("middle_container").innerHTML+="<img src="+dynamicContent.Feed_300x250[0].content['ld_cta.png']['Url']+" id='ld_cta' class='images fade'></img>";
    
    document.getElementById("middle_container").innerHTML+="<img src="+dynamicContent.Feed_300x250[0].content['sheen.png']['Url']+" id='sheen' class='images fade'></img>";
    
    document.getElementById("middle_container").innerHTML+="<img src="+dynamicContent.Feed_300x250[0].content['sky_glass_logo.png']['Url']+" id='sky_glass_logo' class='images fade'></img>";

  document.getElementById("middle_container").innerHTML+="<img src="+dynamicContent.Feed_300x250[0].content['sky_logo.png']['Url']+" id='sky_logo' class='images fade'></img>";
    
    // create a new stage and point it at our canvas:
    stageP = new createjs.Stage(document.getElementById("testCanvas"));
    spriteSheet1 = new createjs.SpriteSheet({
    framerate: 1,
    "images": [dynamicContent.Feed_300x250[0].content['Spritesheet_min.jpg']['Url']],    
    "frames": {
      "height":250,
      "count": 273,
      "width": 300
    ...
    "animations": {
      "frame_1":[0,42, "frame_1_stop"],
      "frame_1_stop": [42],
      "frame_2":[43, 272, "frame_2_stop"],
      "frame_2_stop": [272]
    }
  });
...
function init() {
    ...
    var manifest =  [
		{src:dynamicContent.Feed_300x250[0].content['frame_1_copy.png']['Url'], id:"frame_1_copy"},
		{src:dynamicContent.Feed_300x250[0].content['ld_copy.png']['Url'], id:"ld_copy"}
	]
...
function spriteSheetAnim(){
    ...
    TweenLite.to(sheen, 0.1, {y:205, ease:Quad.easeOut});
    ...
    TweenLite.to(sky_logo,0.2,{alpha:1,ease:Quad.easeOut});
    TweenLite.to(initAnim,0.5,{alpha:1,ease:Quad.easeOut});
...
function anim1EndHandler(){
    ...
    TweenLite.to(f1_copy,0.8,{delay:0.5, alpha:1,ease:Quad.easeOut, onComplete:function(){f1_copy.gotoAndPlay(2);}});
...
{
     TweenLite.to(sky_logo,0.2,{alpha:0,ease:Quad.easeOut});
    TweenLite.to(f1_copy,0.1,{alpha:0,ease:Quad.easeOut});
    TweenLite.to(initAnim,0.1,{alpha:0,ease:Quad.easeOut, onComplete:function(){
        initAnim.gotoAndPlay('frame_2');
    }});
    TweenLite.to(frame_2_copy,0.5,{delay:0.1, alpha:1,ease:Quad.easeOut});
    
    TweenLite.to(initAnim,0.5,{delay:0.1, alpha:1,ease:Quad.easeOut, onComplete:function(){
        //initAnim.addEventListener("animationend",  anim2EndHandler);
    }});
    TweenLite.set(frame_3_copy,{scale:1.2});
...
function showCopy()
{
    ...
    {
        TweenLite.to(frame_2_copy,0.1,{alpha:0,ease:Quad.easeOut});
    }
    
    if(initAnim.currentFrame >= 137 && initAnim.currentFrame < 139)
    {
        if(bool137)
        {
        ...
        TweenLite.to(frame_3_copy, 0.5,{alpha:1, overwrite:0});
        TweenLite.to(frame_3_copy, 3,{scale:0.9, ease:Linear.easeNone, overwrite:0});
     ...
     if(initAnim.currentFrame >= 162 && initAnim.currentFrame <= 206)
    { 
    ...
    if(initAnim.currentFrame >= 207 && initAnim.currentFrame <= 252)
    { 
...
{    
    clearInterval(myVar);
    TweenLite.to(sheen , 0.1, {delay:1.7, alpha:1});
     TweenLite.to(ldframe_copy,0.8,{delay:0.5, alpha:1,ease:Quad.easeOut, onComplete:function(){ldframe_copy.gotoAndPlay(2);}});
     TweenLite.to(ld_cta,0.5,{delay:1.2, alpha:1, ease:Quad.easeOut, onComplete:function(){
        TweenLite.to(sheen, 1, {delay:1,x:180, ease:Quad.easeOut, onComplete:function(){TweenLite.to(sheen , 0.1, {alpha:0})}});
    }});
}
